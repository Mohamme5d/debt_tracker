...
- **Partial payments** — record incremental payments with optional notes; progress bar shows how much is remaining
...
- **Encrypted backup** — AES-256-CBC encrypted backup files (`.rsd`) to local storage or Google Drive
- **Restore from backup** — import from a local `.rsd` file or Google Drive
...
│   │       ├── debt_transaction.dart  # DebtTransaction Isar model
│   │       ├── payment.dart           # Payment Isar model
...
└── features/
    ...
    ├── transactions/
    │   ├── usecases/
    │   │   ├── add_transaction.dart   # AddTransactionUseCase
    │   │   └── record_payment.dart    # RecordPaymentUseCase + deletePayment
    │   ├── providers/
    ...
    │       └── transaction_detail_screen.dart   # Detail, payments, mark settled, delete
    │
    ├── contacts/
    │   ├── providers/
...
- Google Drive (OAuth via `google_sign_in`, multipart upload to Drive API v3)
- iCloud Documents directory (iOS only)

**Import sources:**
- Local `.rsd` file (via file picker)
- Google Drive (lists the most recent `raseed_backup_*` file)
